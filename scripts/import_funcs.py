...
def BGEdata(loc):
	...
	if loc == "new apt":
		...
		elec = elec.append(pd.read_csv('raw_data/new_apt/elec_hourly_2015-11_2015-12.csv',skiprows=0,parse_dates={'timestamp':['DATE','START TIME'],'timestamp_end':['DATE','END TIME']},index_col='timestamp'))
	return elec

if __name__ == "__main__":

	apt = input('Enter "new" or "old" for apartment: ')
	apt = apt + " apt"
	elec = BGEdata(apt)
